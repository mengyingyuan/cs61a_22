...
def about(topic):
    ...
    def select_about(p):
        for word in split(lower(remove_punctuation(p))):
            for w in topic:
                if word == w:
                    return True
        return False
...
def hidden_kittens(typed, reference, limit):
    ...
    def counts(typed, reference):
        ...
        if k == 0:
            ...
            return 0
        if typed[0] == reference[0]:
            return counts(typed[1:], reference[1:]) + counts(typed[1:], reference)
        return counts(typed[1:], reference)
...
def report_progress(typed, prompt, user_id, upload):
    ...
    """
    progress = 0
    while progress < len(typed):
        if typed[progress] == prompt[progress]:
            progress += 1
        else:
            break
...
def fastest_words(match):
    ...
    times =  match["times"]
    words = match["words"]
    fasted = [ [] for _ in player_indices]
    for j in word_indices:
        min, index = times[0][j], 0
        for i in player_indices:
            if min > times[i][j]:
                min, index = times[i][j], i
        fasted[index].append(words[j])
    return fasted
